...
namespace eosiock {
    ...
    */
    ...
    [[nodiscard]] bool rsassa_pss_mgf1_verify(const rsa_public_key_view& rsa_pub_key, const eosio::fixed_bytes<HLen>& digest, const bytes_view& signature, HashT&& hash)
    {
        ...
        // 2. RSA verification
        auto em = rsavp1( rsa_pub_key, signature );
    ...
    * @param error       - error message to use when verification fails
    */
    inline void assert_rsa_pss_sha1(const rsa_public_key_view& rsa_pub_key, const eosio::checksum160& digest, const bytes_view& signature, const char* error) {
        eosio::check( verify_rsa_pss_sha1( rsa_pub_key, digest, signature ), error );
    }
    ...
    * @param error       - error message to use when verification fails
    */
    inline void assert_rsa_pss_sha256(const rsa_public_key_view& rsa_pub_key, const eosio::checksum256& digest, const bytes_view& signature, const char* error) {
        eosio::check( verify_rsa_pss_sha256( rsa_pub_key, digest, signature ), error );
    }
    ...
    * @param error       - error message to use when verification fails
    */
    inline void assert_rsa_pss_sha512(const rsa_public_key_view& rsa_pub_key, const eosio::checksum512& digest, const bytes_view& signature, const char* error) {
        eosio::check( verify_rsa_pss_sha512( rsa_pub_key, digest, signature ), error );
    }
}