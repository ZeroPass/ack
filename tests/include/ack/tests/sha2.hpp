...
namespace ack::tests {
    ...
    namespace detail {
        ...
        inline void write_u32(uint8_t* dest, uint32_t x)
        {
            *dest++ = (x >> 24) & 0xff;
            *dest++ = (x >> 16) & 0xff;
            *dest++ = (x >> 8) & 0xff;
        ...
        inline void write_u64(uint8_t* dest, uint64_t x)
        {
            *dest++ = (x >> 56) & 0xff;
            *dest++ = (x >> 48) & 0xff;
            *dest++ = (x >> 40) & 0xff;
        ...
        inline uint64_t read_u64(const uint8_t* src)
        {
        ...
        inline uint64_t ror(uint64_t x, uint64_t n)
        {
            return (x >> n) | (x << (-n & 63));
        }
        ...
        inline T truncate(const hash_array<N>& hash)
        {
            T result;
        ...
        inline sha256_hash sha256_impl(const uint32_t* s, const uint8_t* data, uint64_t length)
        {
        ...
        inline sha512_hash sha512_impl(const uint64_t* s, const uint8_t* data, uint64_t length)
        {
            static_assert(sizeof(uint32_t) == 4, "sizeof(uint32_t) must be 4");
            static_assert(sizeof(uint64_t) == 8, "sizeof(uint64_t) must be 8");
    ...
    inline sha256_hash sha256(const uint8_t* data, uint64_t length)
    {
        ...
        return detail::sha256_impl(initial_hash_values, data, length);
    }

    inline sha512_hash sha512(const uint8_t* data, uint64_t length)
    {
        ...
        return detail::sha512_impl(initial_hash_values, data, length);
    }
}
