...
namespace ack::tests {
    namespace detail {
        ...
        using secp256k1_t          = decltype(secp256k1);
        using secp256k1_point      = typename secp256k1_t::point_type;
        using secp256k1_point_proj = ec_point_fp_proj_t<typename secp256k1_t::int_type, typename secp256k1_t::curve_tag>;

        using secp256r1_t          = decltype(secp256r1);
        using secp256r1_point      = typename secp256r1_t::point_type;
        using secp256r1_point_proj = ec_point_fp_proj_t<typename secp256r1_t::int_type, typename secp256r1_t::curve_tag>;
        ...
        using c23_point_proj = ec_point_fp_proj_t<decltype(c23)::int_type, decltype(c23)::curve_tag>;
    ...
    EOSIO_TEST_BEGIN( ec_general_test )
        ...
        //       required for generating point is tested in other test cases
        {
            ...
            auto p1_proj = make_ec_point_fp_proj( p1 );
            ...
            p1_proj = make_ec_point_fp_proj( p1 );
            ...
            p1 = c23.make_point( c23.p - 1, c23.p - 1 );
            ...
            p1_proj = c23.generate_point<c23_point_proj>( 1 );
            REQUIRE_EQUAL( p1_proj, make_ec_point_fp_proj( p1 )          ) // Test comparison operator
            REQUIRE_EQUAL( p1_proj != make_ec_point_fp_proj( p1 ), false ) // Test comparison operator
            ...
            REQUIRE_EQUAL( p1_proj == make_ec_point_fp_proj( p1 ), true  ) // Different z coordinate
            REQUIRE_EQUAL( p1_proj != make_ec_point_fp_proj( p1 ), false ) // Test comparison operator
            REQUIRE_EQUAL( p1_proj == c23_point_proj()           , false ) // Test comparison operator
            REQUIRE_EQUAL( p1_proj != c23_point_proj()           , true  ) // Test comparison operator
            ...
            // Test generating point from base point with scalar 7
            ...
            REQUIRE_EQUAL( p1_proj == make_ec_point_fp_proj( p1 ), true  ) // Test comparison operator
            REQUIRE_EQUAL( p1_proj != make_ec_point_fp_proj( p1 ), false ) // Test comparison operator
            REQUIRE_EQUAL( p1_proj == c23_point_proj()           , false ) // Test comparison operator
            REQUIRE_EQUAL( p1_proj != c23_point_proj()           , true  ) // Test comparison operator
            ...
            REQUIRE_EQUAL( p1_inv_proj == make_ec_point_fp_proj( p1_inv ), true  ) // Test comparison operator
            REQUIRE_EQUAL( p1_inv_proj != make_ec_point_fp_proj( p1_inv ), false ) // Test comparison operator
            ...
            REQUIRE_EQUAL( ( p1_inv_proj.x == 5 ) && ( p1_inv_proj.y == 19 ) && ( p1_proj.z == 1 ), true )
            ...
            REQUIRE_EQUAL( -p1_proj, p1_inv_proj ) // Test '-' operator
            REQUIRE_EQUAL( p1_inv_proj == make_ec_point_fp_proj( p1_inv ), true  ) // Different z coordinate
            REQUIRE_EQUAL( p1_inv_proj != make_ec_point_fp_proj( p1_inv ), false ) // Test comparison operator
            REQUIRE_EQUAL( p1_inv_proj == c23_point_proj()               , false ) // Test comparison operator
            REQUIRE_EQUAL( p1_inv_proj != c23_point_proj()               , true  ) // Test comparison operator
    ...
    EOSIO_TEST_BEGIN(ec_pkv_secp256r1_test)
        {
            ...
            // [P-256]
            {
                ...
                REQUIRE_EQUAL( make_ec_point_fp_proj( q ).is_valid(), true )
                ...
                REQUIRE_EQUAL( make_ec_point_fp_proj( q ).is_valid(), false )
    ...
    EOSIO_TEST_BEGIN( ec_double_test )
        ...
        for_each_curve_do( []<typename CurveT>(const CurveT& c) {
            ...
            using point_proj_type = ec_point_fp_proj_t<typename CurveT::int_type, typename CurveT::curve_tag>;
            ...
            p_proj = make_ec_point_fp_proj( c.make_point( 4, 0 ) ).doubled();
            REQUIRE_EQUAL( p_proj, point_proj_type()   )
            REQUIRE_EQUAL( p_proj.is_identity(), true  )
            REQUIRE_EQUAL( p_proj.is_on_curve(), true  )
            REQUIRE_EQUAL( p_proj.is_valid()   , false )
        ...
        // Custom curve tests
        {
            ...
            const auto p1_proj = make_ec_point_fp_proj( p1 );
            const auto r_proj  = make_ec_point_fp_proj( p2 );
    ...
    EOSIO_TEST_BEGIN( ec_add_test )
        ...
        for_each_curve_do( []<typename CurveT>(const CurveT& c) {
            ...
            const auto g_proj = make_ec_point_fp_proj( c.g );
    ...
    EOSIO_TEST_BEGIN(ec_mul_secp256r1_test)
        ...
        // src: https://github.com/randombit/botan/blob/321a50789e6eeda6898af114492445f0882ee70f/src/tests/data/pubkey/ecc_var_point_mul.vec
        {
            ...
            REQUIRE_EQUAL( r, ( make_ec_point_fp_proj( p ) * k ).to_affine() )
    ...
    EOSIO_TEST_BEGIN( ec_mul_add_fast_test )
        ...
        for_each_curve_do( []<typename CurveT>(const CurveT& c) {
            ...
            for ( int32_t i = 1; i < 101; i++) {
                auto p  = c.g * i;
                auto p_proj = make_ec_point_fp_proj( p );
                auto q  = c.g * (101 - i );
                auto q_proj = make_ec_point_fp_proj( q );
                auto u1 = (i * ( c.n - 1 )) % c.n;
                auto u2 = (i * ( c.n - 2 )) % c.n;
