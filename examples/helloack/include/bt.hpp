// Copyright © 2023 ZeroPass <zeropass@pm.me>
// Author: Crt Vavros

#include <array>
#include <ack/bigint.hpp>
#include <ack/ec_curve.hpp>
#include <ack/utils.hpp>

// Benchmark test vectors

struct rsa_1024_sha1 {
    // NIST FIPS 186-4 test vectors
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.0
    // "SigVer PKCS#1 Ver 1.5" information
    // Combinations selected: Mod Size 1024 with SHA-1
    constexpr static auto exp = std::array<byte_t, 3>{ 0xfe, 0x3f, 0xa1 };
    constexpr static auto mod = std::array<byte_t, 128>{
        
    };

    constexpr static auto md = std::array<byte_t, 20>{
        0x9D, 0x71, 0xBB, 0xD0, 0xB0, 0x5E, 0x35, 0xAD, 0x04, 0x39, 0x1D, 0xCD,
        0x70, 0x4A, 0x3E, 0x7C, 0x86, 0xD5, 0x36, 0xB5
    };

    constexpr static auto sig = std::array<byte_t, 128>{
        
    };
};

struct rsa_2048_sha1 {
    // NIST FIPS 186-4 test vectors
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.0
    // "SigVer PKCS#1 Ver 1.5" information
    // Combinations selected: Mod Size 2048 with SHA-1
    constexpr static auto exp = std::array<byte_t, 3>{ 0x0b, 0x8f, 0x69 };
    constexpr static auto mod = std::array<byte_t, 256>{
        
    };

    constexpr static auto md = std::array<byte_t, 20>{
        0x50, 0xBC, 0xB0, 0xA1, 0x72, 0xB9, 0xAB, 0x76, 0x64, 0xC7, 0x97, 0x79,
        0xBE, 0xBE, 0x16, 0x9A, 0x50, 0xAC, 0xC1, 0xCE
    };

    constexpr static auto sig = std::array<byte_t, 256>{
        
    };
};

struct rsa_1024_sha256 {
    // NIST FIPS 186-4 test vectors
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.0
    // "SigVer PKCS#1 Ver 1.5" information
    // Combinations selected: Mod Size 1024 with SHA-256
    constexpr static auto exp = std::array<byte_t, 3>{ 0xee, 0xf2, 0x11 };
    constexpr static auto mod = std::array<byte_t, 128>{
        
    };

    constexpr static auto md = std::array<byte_t, 32>{
        0xF9, 0x62, 0x5E, 0x08, 0x91, 0x9E, 0x66, 0x32, 0xF9, 0x0B, 0x9A, 0x9F,
        0xBD, 0xF8, 0x93, 0x0E, 0x71, 0xCB, 0x17, 0x25, 0xB8, 0x33, 0x48, 0xF9,
        0x39, 0x05, 0xC7, 0xB2, 0x02, 0x9C, 0x91, 0xC7
    };

    constexpr static auto sig = std::array<byte_t, 128>{
        
    };
};

struct rsa_2048_sha256 {
    // NIST FIPS 186-4 test vectors
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.0
    // "SigVer PKCS#1 Ver 1.5" information
    // Combinations selected: Mod Size 2048 with SHA-256
    constexpr static auto exp = std::array<byte_t, 3>{ 0x49, 0xd2, 0xa1 };
    constexpr static auto mod = std::array<byte_t, 256>{
        
    };

    constexpr static auto md = std::array<byte_t, 32>{
        0x56, 0x5F, 0xF4, 0xF3, 0x6E, 0x8B, 0xD4, 0xA9, 0x60, 0x07, 0xED, 0x57,
        0x7F, 0x32, 0x48, 0xB4, 0xF9, 0x94, 0x38, 0x26, 0xD7, 0x21, 0xB4, 0x17,
        0xA2, 0x0B, 0xB1, 0x2C, 0x3A, 0xE6, 0xE8, 0x74
    };

    constexpr static auto sig = std::array<byte_t, 256>{
        
    };
};

struct rsa_pss_mgf1_sha256_2048 {
    // NIST FIPS 186-4 test vectors
    // Generated from: 'SigVerPSS_186-3.rsp'
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.1
    // "FIPS186-3 - SigVer RSA PKCS#1 RSASSA-PSS" information for "rsa2_check"
    // Combinations selected:  Mod Size 2048 with SHA-256(Salt len: 32) ;

    constexpr static size_t salt_len = 32;
    constexpr static auto exp = fixed_bytes<3>{ 0x10, 0xe4, 0x3f };
    constexpr static auto mod = fixed_bytes<256>{
        
    };

    constexpr static auto md = fixed_bytes<32>{
        0x09, 0xE6, 0x05, 0xCA, 0xCE, 0xE4, 0x06, 0x3A, 0x2F, 0xA3, 0xB2, 0xBB,
        0x6B, 0x3C, 0x90, 0x1B, 0x81, 0xFB, 0x98, 0x78, 0xDA, 0x41, 0x1F, 0xBB,
        0xFF, 0x44, 0x26, 0x22, 0x10, 0xAA, 0xAC, 0xE4
    };

    constexpr static auto sig = fixed_bytes<256> {
        
    };
};

struct rsa_4096_sha256 {
    // Custom test vector
    constexpr static auto exp = fixed_bytes<3>{ 0x01, 0x00, 0x01 };
    constexpr static auto mod = fixed_bytes<512>{
        
    };

    constexpr static auto md = fixed_bytes<32>{
        0x42, 0x95, 0xCF, 0x86, 0xE1, 0x1B, 0x13, 0x7A, 0x6A, 0xC2, 0x31, 0xA0,
        0xB0, 0xC1, 0xD1, 0x2B, 0x19, 0x69, 0x9E, 0x3F, 0xC6, 0x99, 0xD8, 0x9C,
        0xF2, 0x04, 0x5D, 0xDE, 0x30, 0x38, 0x00, 0x98
    };

    constexpr static auto sig = fixed_bytes<512>{
        
    };
};

struct rsa_pss_mgf1_sha256_4096 {
    // NIST FIPS 186-4 test vectors
    // Generated from: 'SigVerPSS_186-3.rsp'
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.1
    // "FIPS186-3 - SigVer RSA PKCS#1 RSASSA-PSS" information for "rsa2_check"
    // Combinations selected:  Mod Size 4096 with SHA-256(Salt len: 32) ;

    constexpr static size_t salt_len = 32;
    constexpr static auto exp = fixed_bytes<3>{ 0x01, 0x00, 0x01 };
    constexpr static auto mod = fixed_bytes<512>{
        
    };

    constexpr static auto md = fixed_bytes<32> {
        0x94, 0x32, 0xC1, 0xA7, 0xD3, 0x43, 0xFC, 0xFA, 0xCB, 0x16, 0x4B, 0xDC,
        0x44, 0xFF, 0x71, 0xC1, 0x28, 0x1C, 0x00, 0x48, 0x86, 0xB1, 0xC4, 0x28,
        0x41, 0x90, 0x88, 0xD0, 0x6C, 0xD3, 0x56, 0x1A
    };

    constexpr static auto sig = fixed_bytes<512>{
        
    };
};

struct rsa_1024_sha512 {
    // NIST FIPS 186-4 test vectors
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.0
    // "SigVer PKCS#1 Ver 1.5" information
    // Combinations selected: Mod Size 1024 with SHA-512
    constexpr static auto exp = fixed_bytes<3>{ 0xd4, 0x3d, 0x27 };
    constexpr static auto mod = fixed_bytes<128>{
        
    };

    constexpr static auto md = fixed_bytes<64>{
        0x27, 0xF4, 0xE0, 0xD2, 0xA3, 0x32, 0x7B, 0xFF, 0xE9, 0xC3, 0xA5, 0x02,
        0x38, 0x00, 0x3F, 0xBF, 0xDD, 0x18, 0x81, 0x9D, 0x56, 0x78, 0x79, 0x19,
        0x05, 0x84, 0x69, 0x63, 0x08, 0xF8, 0xFD, 0xE0, 0x04, 0x5F, 0xEA, 0x06,
        0x9F, 0xE6, 0x1D, 0x89, 0xB5, 0xFF, 0xF7, 0x81, 0x33, 0x88, 0x0A, 0xDD,
        0xA4, 0x1B, 0xD7, 0x12, 0x7A, 0x15, 0x4C, 0xEE, 0xBE, 0x6C, 0x01, 0xFE,
        0x1F, 0x50, 0x64, 0x36
    };

    constexpr static auto sig = fixed_bytes<128>{
        
    };
};

struct rsa_2048_sha512 {
    // NIST FIPS 186-4 test vectors
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.0
    // "SigVer PKCS#1 Ver 1.5" information
    // Combinations selected: Mod Size 2048 with SHA-512
    constexpr static auto exp = fixed_bytes<3>{ 0xa3, 0xe1, 0x87 };
    constexpr static auto mod = fixed_bytes<256>{
        
    };

    constexpr static auto md = fixed_bytes<64>{
        0x79, 0x50, 0xF8, 0x74, 0x43, 0x54, 0xA1, 0xEC, 0x6F, 0xA4, 0x63, 0xAD,
        0xC4, 0x29, 0x56, 0x29, 0x85, 0xB9, 0x4E, 0x64, 0xC8, 0xF2, 0xE5, 0xD7,
        0x16, 0xF1, 0xC3, 0xB1, 0x55, 0x6D, 0xBA, 0x31, 0xBB, 0x29, 0x0E, 0x89,
        0xE8, 0x89, 0x67, 0x95, 0x7F, 0xBC, 0xBC, 0x15, 0x33, 0xDC, 0x23, 0x00,
        0xCE, 0xC2, 0x73, 0xB0, 0x99, 0x0B, 0x7C, 0x93, 0x87, 0x51, 0x57, 0x25,
        0x7E, 0xE4, 0x5F, 0x37
    };

    constexpr static auto sig = fixed_bytes<256>{
        
    };
};

struct secp256k1_sha256_tv
{
    // Test vector from Google's Wycheproof RSA signature verification tests.
    // Generated from: 'ecdsa_secp256k1_sha256_p1363_test.json'
    // URL: 'https://raw.githubusercontent.com/google/wycheproof/master/testvectors/ecdsa_secp256k1_sha256_p1363_test.json'
    //
    // Algorithm: ECDSA
    // GeneratorVersion: 0.8r12

    constexpr static auto& curve = ack::ec_curve::secp256k1;
    using point_type = std::remove_cvref_t<decltype(curve)>::point_type;
    using int_type = std::remove_cvref_t<decltype(curve)>::int_type;

    static constexpr auto pub_point = curve.make_point(
        "00b838ff44e5bc177bf21189d0766082fc9d843226887fc9760371100b7ee20a6f",
        "00f0c9d75bfba7b31a6bca1974496eeb56de357071955d83c4b1badaa0b21832e9"
    );

    // special case hash
    static constexpr auto h     = from_hex( "9b6cd3b812610000000026941a0f0bb53255ea4c9fd0cb3426e3a54b9fc6965c" );
    static constexpr int_type r = "2bdea41cda63a2d14bf47353bd20880a690901de7cd6e3cc6d8ed5ba0cdb1091";
    static constexpr int_type s = "c31599433036064073835b1e3eba8335a650c8fd786f94fe235ad7d41dc94c7a";
};

struct secp256r1_sha256_tv {
    // NIST FIPS 186-4 test vectors
    // https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/digital-signatures
    // CAVS 11.0
    // "SigVer" information
    // Curves/SHAs selected: P-256,SHA-256
    // Generated on Wed Mar 16 16:16:55 2011

    constexpr static auto& curve = ack::ec_curve::secp256r1;
    using point_type = std::remove_cvref_t<decltype(curve)>::point_type;
    using int_type = std::remove_cvref_t<decltype(curve)>::int_type;

    static constexpr auto pub_point = curve.make_point(
        "e424dc61d4bb3cb7ef4344a7f8957a0c5134e16f7a67c074f82e6e12f49abf3c",
        "970eed7aa2bc48651545949de1dddaf0127e5965ac85d1243d6f60e7dfaee927"
    );

    static constexpr auto     h = from_hex( "d1b8ef21eb4182ee270638061063a3f3c16c114e33937f69fb232cc833965a94" );
    static constexpr int_type r = "bf96b99aa49c705c910be33142017c642ff540c76349b9dab72f981fd9347f4f";
    static constexpr int_type s = "17c55095819089c2e03b9cd415abdf12444e323075d98f31920b9e0f57ec871c";
};
